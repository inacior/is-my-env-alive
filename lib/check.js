...
const _getURLResponse = async(url) => {
  ...
  try {
    ...
    else if (error.code) response = { status: error.code }
...
const check = async ({ env, detailed=false, file }) => {
  spinnies.add('global', { text: 'Buscando APIs... \n', color: 'gray', succeedColor: 'gray' });
	...
	const callChain = Object.entries(projects).map(async([
		name,
		options
	]) => {
    ...
    spinniesHandler.call(spinnies, name, { text: `${name} [${response.status}]` })
